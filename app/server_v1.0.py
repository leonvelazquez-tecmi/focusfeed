...
from app.storage import (
    ...
    ensure_seed_if_empty, get_pending_item_ids, bulk_set_status
)
...
class FeedCuratorHTTPHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == "/" or path == "/index.html":
            ...
            try:
                data = fetch_items(tab=tab, type_filter=type_filter, limit=limit)
    ...
    def do_POST(self):
        ...
        # Bulk status (Marcar todo como leido)
        if path == "/api/items/bulk-status":
            ids = body.get("ids", [])
            new_status = body.get("status", "archived")
            try:
                updated = bulk_set_status(ids, new_status)
