...
def fetch_items(tab="curated", type_filter="all", limit=120):
    if is_supabase():
        query = "items?select=*"
        if tab == "curated":
...
def bulk_set_status(item_ids: list, new_status: str):
    """Cambia el status de varios items en una sola llamada (Marcar todo como leido)."""
    ids = [int(i) for i in item_ids if str(i).isdigit() or isinstance(i, int)]
    if not ids:
        return 0
    if is_supabase():
        id_list = ",".join(str(i) for i in ids)
        res = supabase_request(f"items?id=in.({id_list})", method="PATCH", data={"status": new_status})
        if isinstance(res, list):
            return len(res)
        err_msg = res.get("message") if isinstance(res, dict) else str(res)
        raise RuntimeError(f"Supabase rechazo el cambio de status masivo: {err_msg}")
    else:
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.executemany("UPDATE items SET status = ? WHERE id = ?", [(new_status, i) for i in ids])
        conn.commit()
        conn.close()
        return len(ids)
...
def batch_save_items(items: list):
    ...
    if is_supabase():
        ...
        # row a second time). Dedup por guid antes de enviar, quedandonos con la
        # ultima version (mas reciente/enriquecida) de cada item.
        by_guid = {}
        for item in items:
            by_guid[item["guid"]] = item

        # CRITICO: solo se insertan items nuevos. Antes se hacia upsert con
        # status='inbox' y eso resucitaba en cada sync todo lo marcado como
        # visto/guardado, ademas de pisar el analisis de IA con los defaults.
        ...
        # on_conflict=guid con ignore-duplicates: si otro sync concurrente ya lo
        # metio, no se pisa la fila existente.
        ...
        err_msg = res.get("message") if isinstance(res, dict) else str(res)
        raise RuntimeError(f"Supabase rechazo la insercion de items: {err_msg}")
    else:
